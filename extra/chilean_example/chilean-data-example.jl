...
R"""
data <-
    'extra/chilean_example/data.xlsx' |>
...
readr::write_csv(x = data, file = "extra/chilean_example/cleaned_data.xlsx")
"""
...
begin
    ...
    for p in [2, 4]
        ...
        filename = "extra/chilean_example/gamma-var-$p.csv"
        R"""
        $df |>
...
R"""
df <-
    "extra/chilean_example" |>
    list.files("gamma-var*", full.names = TRUE) |>
...
fig |>
    ggplot2::ggsave(
        filename = "extra/chilean_example/fig-prob-1vs1.png",
        dpi = 1200,
        height = 3.5,
        width = 5.5,
    )
...
fig |>
    ggplot2::ggsave(
        filename = "extra/chilean_example/fig-prob-2vs1-investment.png",
        height = 3.5,
        width = 5.5,
        dpi = 1200,
    )
...
readr::write_csv(
  x = pvals,
  file = "C:/Users/Ivan/Documents/julia-projects/BNPVAR.jl/extra/chilean_example/pvals-1vs1.csv"
)
"""
...
fig <-
    "extra/chilean_example" |>
    list.files("pvals-1vs1-var*", full.names = TRUE) |>
    purrr::map_df(readr::read_csv) |>
    dplyr::rename(`# lags` = nlags) |>
    ggplot2::ggplot(
        ggplot2::aes(
...
fig |>
    ggplot2::ggsave(
        filename = "extra/chilean_example/fig-pval-1vs1.png",
        dpi = 1200,
        height = 3.5,
        width = 5.5,
    )
